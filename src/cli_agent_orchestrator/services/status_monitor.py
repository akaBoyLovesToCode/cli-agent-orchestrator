...
# How long after a turn's keystrokes are DELIVERED a ready reading may close that
# turn without the agent ever having been seen working (#735).
#
# Every ordinary turn is observed working, so this only governs the turn that
# finishes inside one detection cycle or renders no activity frame at all. Both
# alternatives to a grace are worse: closing the turn immediately is the defect
# itself (the frame is indistinguishable from the PREVIOUS turn's), and never
# closing it converts a wrong answer into a hang. Measured from delivery rather
# than from ``notify_input_sent`` because a provider's ``paste_submit_delay``
# sits between the two (2s for claude_code), and during it the agent has not yet
# been handed the prompt — a grace spent there would guard nothing.
#
# 3.0s is deliberately the same floor as the flat ``time.sleep(3)`` the CLI
# callers used to rely on, so no caller gets slower than it was before.
TURN_START_GRACE_S = 3.0
...
class StatusMonitor:
    ...
    def __init__(self):
        ...
        # when nothing has been dispatched yet. Only TURN_START_GRACE_S reads it.
    ...
    def _apply_detection_locked(
        ...
        *,
        ...
        if last == TerminalStatus.PROCESSING and detected in _STICKY_READY_STATUSES:
            if not settled:
                logger.debug(
    ...
    def _turn_unstarted_locked(self, terminal_id: str) -> bool:
        ...
        TURN_START_GRACE_S has passed since the keystrokes landed — whichever comes
        ...
        if delivered is None:
            return False
        return time.monotonic() - delivered < TURN_START_GRACE_S
    ...
    def _note_turn_progress_locked(
        ...
        The TURN_START_GRACE_S branch is the one escape, for a turn that finishes
        inside a single detection cycle and so is never seen working; without it that
        turn would never close.
        """
        if detected in _ACTIVE_STATUSES:
            self._turn_started[terminal_id] = True
            return
        if detected not in _STICKY_READY_STATUSES or not settled:
        ...
        if not self._turn_started.get(terminal_id, False):
            logger.debug(
                f"_note_turn_progress [{terminal_id}]: closing turn {turn} on a settled "
                f"{detected.value} without having seen it work (grace elapsed)"
            )
    ...
    def notify_input_delivered(self, terminal_id: str) -> None:
        ...
        this replaces it with the real one. Only TURN_START_GRACE_S reads it, and its
        definition explains why the grace cannot start at the arm instead.
        """
        with self._lock:
            self._turn_delivered_at[terminal_id] = time.monotonic()
