...
class TestLiveTurnIsNotDemotedByAnUnsettledFrame:
    ...
    def test_the_unstarted_window_expires_so_nothing_can_hang_on_it(self):
        sm = StatusMonitor()
        sm._last_status["t1"] = TerminalStatus.COMPLETED
        sm.notify_input_sent("t1", assume_processing=True)
        with sm._lock:
            sm._turn_delivered_at["t1"] = time.monotonic() - (TURN_START_GRACE_S + 0.1)
        sm._apply_detection("t1", TerminalStatus.COMPLETED, settled=True)
        assert sm._last_status["t1"] == TerminalStatus.COMPLETED
...
class TestTurnCorrelation:
    ...
    def test_a_turn_never_seen_working_closes_once_the_grace_elapses(self):
        """Otherwise a turn that finishes inside one detection cycle would never
        close and the caller would hang instead of getting a wrong answer."""
        sm = StatusMonitor()
        sm.notify_input_sent("t1")
        sm.notify_input_delivered("t1")
        with sm._lock:
    ...
    def test_delivery_pushes_the_grace_back(self):
        ...
        with sm._lock:  # pretend the arm happened a long time ago
            sm._turn_delivered_at["t1"] = time.monotonic() - (TURN_START_GRACE_S + 5)
